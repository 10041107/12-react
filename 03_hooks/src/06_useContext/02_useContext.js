...
하지만 context를 사용하면 중간 컴포넌트들에게 props를 넘겨주지 않아도 되고, 유지보수도 훨신 수월해지게 된다.
단, context를 사용하면 컴포넌트를 재사용하기 어려워지기 때문에 꼭 필요할 때만 써야 한다.
따라서 때에 따라서는 context보다 props drilling이 더 간단한 해결책이 될 수 있다.

context객체를 createContext를 통해 만들게 되고 defavltValue가 없는 경우에는 null로 설정할 수 있다.
context객체를 구독하고 있는 컴포넌트를 렌더링 할 때 react는 트리 상위에서 가장 가까이 있는
Provider로부터 현재 값을 읽어들인다. 이때 적절한 provider를 찾지 못할 때 쓰이는 값이 defaultValue이다.
*/
